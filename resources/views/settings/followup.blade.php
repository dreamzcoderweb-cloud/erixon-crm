...
@section('title', 'Followup Settings - Settings')
...
<div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="col-md-12">
            <ul class="nav nav-pills flex-column flex-md-row gap-2">
                ...
                @can('followup-settings.view')
                    <li class="nav-item">
                        <a class="nav-link active" href="{{ route('admin.settings.followup') }}">
    ...
    <!-- Followup Settings Vertical Layout -->
    <div class="row">
                ...
                <div class="card-body p-3">
                    <div class="nav flex-column nav-pills custom-lead-nav gap-1" id="v-pills-tab" role="tablist" aria-orientation="vertical">
                        <button class="nav-link text-start py-2 px-3 fw-medium active" id="v-pills-additional-fields-tab" data-bs-toggle="pill" data-bs-target="#v-pills-additional-fields" type="button" role="tab">
                            <i class="bx bx-list-plus me-2"></i> Additional Followup Fields
                        </button>
                        <button class="nav-link text-start py-2 px-3 fw-medium" id="v-pills-site-customization-tab" data-bs-toggle="pill" data-bs-target="#v-pills-site-customization" type="button" role="tab">
                            <i class="bx bx-customize me-2"></i> Followup List Customization
                        </button>
                    </div>
                </div>
            </div>
        ...
        <div class="col-md-9">
            <div class="tab-content p-0 shadow-none bg-transparent" id="v-pills-tabContent">
                ...
                <!-- Additional Followup Fields (Default Active) -->
                <div class="tab-pane fade show active" id="v-pills-additional-fields" role="tabpanel">
                    <div class="card shadow-sm border-0">
                        <div class="card-header border-bottom d-flex align-items-center justify-content-between py-3">
                            <div>
                                <h5 class="card-title mb-0 fw-bold text-dark">Custom Followup Fields</h5>
                                <span class="text-muted small">Additional Followup Fields</span>
                            </div>
                            @can('followup-settings.edit')
                ...
                <div class="tab-pane fade" id="v-pills-site-customization" role="tabpanel">
                    <div class="card shadow-sm border-0 rounded-3">
                        <div class="card-header border-bottom bg-white d-flex align-items-center justify-content-between py-3 px-4">
                            <h5 class="card-title mb-0 fw-bold text-dark fs-5">Followup List Customization</h5>
                            @can('followup-settings.edit')
                                <button type="button" class="btn btn-primary fw-semibold shadow-sm" id="saveFollowupListColumnsBtn">
            ...
            <form id="createCustomFieldForm" method="POST">
                ...
                <div class="modal-body py-4">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" name="field_label" id="field_label" placeholder="e.g. Followup Outcome, Preferred Time" required>
            ...
            <form id="editCustomFieldForm" method="POST">
                ...
                <div class="modal-body py-4">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" name="field_label" id="edit_field_label" placeholder="e.g. Followup Outcome" required>
                        <div class="invalid-feedback"></div>
                    </div>
                    <div class="mb-3">
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        $('#createCustomFieldForm').on('submit', function (e) {
            ...
            $.ajax({
                url: "{{ route('admin.settings.followup.custom-fields.store') }}",
                type: "POST",
                data: form.serialize(),
        ...
        $(document).on('click', '.btn-edit-field', function () {
            ...
            $.ajax({
                url: APP_URL + '/admin/settings/followup/custom-fields/edit/' + fieldId,
                type: 'GET',
                success: function (response) {
                    if (response.status) {
        ...
        $('#editCustomFieldForm').on('submit', function (e) {
            ...
            $.ajax({
                url: APP_URL + '/admin/settings/followup/custom-fields/update/' + fieldId,
                type: 'POST',
                data: form.serialize(),
                success: function (response) {
                    if (response.status) {
        ...
        $('#confirmDeleteFieldBtn').on('click', function () {
            ...
            $.ajax({
                url: APP_URL + '/admin/settings/followup/custom-fields/delete/' + deleteFieldId,
                type: 'DELETE',
                success: function (response) {
                    if (response.status) {
        ...
        /* --- Followup List Customization (Drag & Drop Horizontal Chips + 2-Col Checkboxes) --- */
        let dragItem = null;
        ...
        $('#saveFollowupListColumnsBtn').on('click', function () {
            let columns = [];
            ...
            $.ajax({
                url: "{{ route('admin.settings.followup.list-columns.save') }}",
                type: "POST",
                data: {
                ...
                error: function () {
                    showSettingsAlert('danger', 'Failed to save Followup List customization.');
                },
                complete: function () {
                    btn.prop('disabled', false);
                    spinner.addClass('d-none');
