...
@section('title', 'Edit Role - Super Admin')
...
<div class="container-xxl flex-grow-1 container-p-y">
    ...
    <div class="d-flex flex-wrap align-items-center justify-content-between gap-2 mb-4">
        <div>
            ...
            <h4 class="fw-bold mb-0 text-dark">Edit Role: <span class="text-primary">{{ $role->name }}</span></h4>
        </div>
    ...
    <form action="{{ url('admin/edit_role/' . $role->id) }}" method="POST" id="roleForm">
        @csrf
                    ...
                    <div class="col-md-8">
                        ...
                        <div class="input-group">
                            ...
                            <input type="text" class="form-control border-start-0 ps-2 @error('name') is-invalid @enderror"
                                id="name" name="name" value="{{ old('name', $role->name) }}" placeholder="e.g. Sales Manager, Accountant, HR Admin" required />
                        </div>
                        @error('name')
                            ...
                            <div class="row g-2">
                                @foreach($modulePerms as $perm)
                                    @php
                                        ...
                                        $isChecked = in_array($perm->id, old('permissions', $selectedPermissionIds ?? []));
                                    @endphp
        ...
        <div class="card role-form-card mt-4 mb-4">
            <div class="card-body py-3 d-flex align-items-center justify-content-between">
                <span class="text-muted small">Double-check all updated permissions before saving changes.</span>
                <div class="d-flex gap-2">
                    <a href="{{ route('admin.roles.index') }}" class="btn btn-outline-secondary px-4 rounded-pill">Cancel</a>
                    <button type="submit" class="btn btn-primary px-4 rounded-pill shadow-sm">
                        <i class="bx bx-check-circle me-1"></i> Update Role
                    </button>
                </div>
            </div>
        </div>
